...
def FDTD_1D(Tmax, courant_number, dx, xmax, epsR_func, k1, k2, source_func, ks):
    ...
    for i in range(nt-1):
        ...
        cb[k3:k1] = courant_number/epsR_02
        cb[k2:k4] = courant_number/epsR_02
...
def anim2_E_H(t, x, E, H, k1, k2, y_low, y_high, interval=1E-3, show=True, save=False):
    ...
    plt.fill_betweenx([y_low, y_high], x1=x[k1],
                      x2=x[k2], color="grey", alpha=0.8)
    plt.fill_betweenx([y_low,y_high],x1=x[k3],x2=x[k1],color="red",alpha=0.5)
    plt.fill_betweenx([y_low,y_high],x1=x[k2],x2=x[k4],color="red",alpha=0.5)
    plt.ylim(y_low, y_high)
    plt.xlim(0, xmax)
